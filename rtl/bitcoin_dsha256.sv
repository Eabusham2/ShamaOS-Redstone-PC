module bitcoin_dsha256 (
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            core_state_in <= IV; core_block <= 0; first_digest <= 0; phase <= 0;
        end else begin
            done <= 0;
            core_start <= 0;
            case (phase)
                0: if (start) begin
                    busy <= 1;
                    core_state_in <= IV;
                    core_block <= header[639:128];
                    core_start <= 1;
                    phase <= 1;
                end
                1: if (core_done) begin
                    core_state_in <= core_state_out;
                    core_block <= {header[127:0], 8'h80, 312'd0, 64'd640};
                    core_start <= 1;
                    phase <= 2;
                end
                2: if (core_done) begin
                    first_digest <= core_state_out;
                    core_state_in <= IV;
                    core_block <= {core_state_out, 8'h80, 184'd0, 64'd256};
                    core_start <= 1;
                    phase <= 3;
                end
                3: if (core_done) begin
